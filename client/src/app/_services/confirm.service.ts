...
export class ConfirmService {
          ...
          btnOkText='Ok', btnCancelText='Cancel'):Observable<boolean>
  {
    ...
    this.bdModalRef = this.modalService.show(ConfirmDialogComponent,config);
    return new Observable<boolean>(this.getResult());
  }

  private getResult(){
    return (observer) =>{
      const subscription = this.bdModalRef.onHidden.subscribe(()=>{
        observer.next(this.bdModalRef.content.result);
        observer.complete();
      });
      return{
        unsubscribe(){
